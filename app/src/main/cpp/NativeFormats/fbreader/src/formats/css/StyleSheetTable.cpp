...
shared_ptr<ZLTextStyleEntry> StyleSheetTable::createOrUpdateControl(const AttributeMap &styles, shared_ptr<ZLTextStyleEntry> entry) {
    ...
    ZLLogger::Instance().registerClass("imageSize");
	const std::string &width = value(styles, "width");
	if (!width.empty()  && width != "auto") {
        ZLLogger::Instance().println("imageSize", "width ="  + width );
		short size = 100;
		ZLTextStyleEntry::SizeUnit unit = ZLTextStyleEntry::SIZE_UNIT_PERCENT;
		if (parseLength(width, size, unit)){
			entry->setLengthWidth(size, unit);
		}
	}

	const std::string &height = value(styles, "height");
	if (!height.empty() && height != "auto") {
        ZLLogger::Instance().println("imageSize", "height ="  + height );
		short size = 100;
		ZLTextStyleEntry::SizeUnit unit = ZLTextStyleEntry::SIZE_UNIT_PERCENT;
		if (parseLength(height, size, unit)){
			entry->setLengthHeight(size, unit);
		}
	}

	const std::string &backgroundImage = value(styles, "background-image");
	if (!backgroundImage.empty()) {
        ZLLogger::Instance().println("imageSize", "backgroundImage="  + backgroundImage );
	}
