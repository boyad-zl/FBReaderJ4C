...
class XHTMLTagSpanAction : public XHTMLTextModeTagAction {
public:
	void doAtStart(XHTMLReader &reader, const char **xmlattributes);
	void doAtEnd(XHTMLReader &reader);
};
...
//    imgFile = new ZLFile(fullfileName);
	if (!imageFile.exists()) {
		return;
	}
...
void XHTMLTagImageAction::doAtEnd(XHTMLReader &reader) {
//    if(imgFile->exists()) {
//        ZLFile zlFile = *imgFile;
//
//    }
}
...
void XHTMLTagSpanAction::doAtStart(XHTMLReader &reader, const char **xmlattributes) {
	bookReader(reader).addControl(SPAN, true);

}

void XHTMLTagSpanAction::doAtEnd(XHTMLReader &reader) {
	bookReader(reader).addControl(SPAN, false);
}
