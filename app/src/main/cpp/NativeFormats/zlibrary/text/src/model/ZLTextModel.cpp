...
void ZLTextModel::addStyleEntry(const ZLTextStyleEntry &entry, const std::vector<std::string> &fontFamilies, unsigned char depth) {
	...
	// modify by Boyad ,add with and height attribute
	if (entry.isFeatureSupported(ZLTextStyleEntry::LENGTH_WIDTH)) {
		len += 4;
	}
	if (entry.isFeatureSupported(ZLTextStyleEntry::LENGTH_HEIGHT)) {
		len += 4;
	}
	// --- calculating entry size
...
*/
	...
	*address++ = depth;
	...
	// modify by Boyad ,add with and height attribute
	if (entry.isFeatureSupported(ZLTextStyleEntry::LENGTH_WIDTH)) {
		const ZLTextStyleEntry::LengthType &len = entry.widthLength;
		address = ZLCachedMemoryAllocator::writeUInt16(address, len.Size);
		*address++ = len.Unit;
		*address++ = 0;
	}
	if (entry.isFeatureSupported(ZLTextStyleEntry::LENGTH_HEIGHT)) {
		const ZLTextStyleEntry::LengthType &len = entry.heightLength;
		address = ZLCachedMemoryAllocator::writeUInt16(address, len.Size);
		*address++ = len.Unit;
		*address++ = 0;
	}
