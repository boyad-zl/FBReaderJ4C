...
public:
	...
	enum Feature {
		...
		LENGTH_WIDTH =                       NUMBER_OF_LENGTHS + 5, // 14; max = 15
		LENGTH_HEIGHT =                      NUMBER_OF_LENGTHS + 6  //  15; max = 15
	};
...
public:
	...
	void setLengthWidth(short length, SizeUnit unit);
	void setLengthHeight(short length, SizeUnit unit);
...
private:
	...
	LengthType widthLength;
	LengthType heightLength;
...
// 设置width 和height属性
inline void ZLTextStyleEntry::setLengthWidth(short length, SizeUnit unit){
	myFeatureMask |= 1 << LENGTH_WIDTH;
	widthLength.Size = length;
	widthLength.Unit = unit;
};

inline void ZLTextStyleEntry::setLengthHeight(short length, SizeUnit unit){
	myFeatureMask |= 1 << LENGTH_HEIGHT;
	heightLength.Size = length;
	heightLength.Unit = unit;
};
