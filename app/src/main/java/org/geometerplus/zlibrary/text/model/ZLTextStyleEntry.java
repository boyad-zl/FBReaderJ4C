...
public abstract class ZLTextStyleEntry {
	public interface Feature {
		...
		int LENGTH_WIDTH                       = NUMBER_OF_LENGTHS + 5;
		int LENGTH_HEIGHT                      = NUMBER_OF_LENGTHS + 6;
	}
	...
	private Length widthLength ;
	private Length heightLength ;
	...
	public final void setWidthLength(short size, byte unit) {
		Log.i("boyad", "setWidthLength: size =" + size + ", unit = " + unit);
		myFeatureMask |= 1 << Feature.LENGTH_WIDTH;
		widthLength = new Length(size, unit);
	}

	public final void setHeightLength( short size, byte unit) {
		Log.i("boyad", "setHeightLength: size =" + size + ", unit = " + unit);
		myFeatureMask |= 1 << Feature.LENGTH_HEIGHT;
		heightLength = new Length(size, unit);
	}

	public final int getWidthLength(ZLTextMetrics metrics, int fontSize) {
		if (widthLength == null) return -1;
		return compute(widthLength, metrics, fontSize, Feature.LENGTH_WIDTH);
	}

	public final int getHeightLength(ZLTextMetrics metrics, int fontSize) {
		if (heightLength == null) return -1;
		return compute(heightLength, metrics, fontSize, Feature.LENGTH_HEIGHT);
	}
