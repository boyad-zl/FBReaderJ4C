...
public abstract class ZLTextDecoratedStyle extends ZLTextStyle {
	...
	private int myLengthWidth;
	private int myLengthHeight;
	...
	private void initMetricsCache(ZLTextMetrics metrics) {
		...
		myLengthWidth = getLengthWidthInternal(metrics, myFontSize);
		myLengthHeight = getLengthHeightInternal(metrics, myFontSize);
	}
	...
	@Override
	public final int getLengthWidth(ZLTextMetrics metrics) {
		if (!metrics.equals(myMetrics)) {
			initMetricsCache(metrics);
		}
		return myLengthWidth;
	}
	protected abstract int getLengthWidthInternal(ZLTextMetrics metrics, int fontSize);

	@Override
	public final int getLengthHeight(ZLTextMetrics metrics) {
		if (!metrics.equals(myMetrics)) {
			initMetricsCache(metrics);
		}
		return myLengthHeight;
	}
	protected abstract int getLengthHeightInternal(ZLTextMetrics metrics, int fontSize);
}
