...
public class ZLTextExplicitlyDecoratedStyle extends ZLTextDecoratedStyle implements ZLTextStyleEntry.Feature, ZLTextStyleEntry.FontModifier {
	...
	@Override
	protected int getLengthWidthInternal(ZLTextMetrics metrics, int fontSize) {

		if (!myEntry.isFeatureSupported(LENGTH_WIDTH)) {
			return Parent.getLengthWidth(metrics);
		}
		return myEntry.getWidthLength(metrics, fontSize);
	}

	@Override
	protected int getLengthHeightInternal(ZLTextMetrics metrics, int fontSize) {

		if (!myEntry.isFeatureSupported(LENGTH_HEIGHT)) {
			return Parent.getLengthHeight(metrics);
		}
		return myEntry.getHeightLength(metrics, fontSize);
	}
}
